...
COMPARE_PROPERTIES = ["fieldtype", "label", "reqd", "hidden", "read_only"]
...
def get_latest_snapshot_doctype(doctype_name):
	snapshots = frappe.get_all(
		...
		filters={"source_type": "DocType", "document_name": doctype_name},
...
def clear_old_differences(doctype_name):
	old = frappe.get_all("Field Difference Log", filters={"doctype_name": doctype_name}, pluck="name")

	for name in old:
		frappe.delete_doc("Field Difference Log", name, ignore_permissions=True, force=True)
