...
def get_exportable_apps() -> list[str]:
	...
	return sorted({app_name for app_name in module_apps if app_name and app_name in installed_apps})
...
def _reconcile_doctype_customization(
	...
	existing_cf = [row for row in (existing_data.get("custom_fields") or []) if isinstance(row, dict)]
	existing_ps = [row for row in (existing_data.get("property_setters") or []) if isinstance(row, dict)]

	# The set of fieldnames this DocType knows about in the database.
	db_known = {fn for fn in frappe.get_all("Custom Field", filters={"dt": doctype}, pluck="fieldname") if fn}
	db_known |= {
		fn
	...
	if include_doctype_property_setters:
		...
		property_setters += [ps for ps in existing_ps if ps.get("doctype_or_field") != "DocField"]
	...
	if with_permissions:
		...
		custom_perms = [r for r in (existing_data.get("custom_perms") or []) if isinstance(r, dict)]
...
def export_selected_customizations(
	...
	if conflicts:
		lines = "".join(
			_("<li><b>{0}</b> ({1}) — already in: {2}</li>").format(label, fieldname, ", ".join(other_apps))
	...
	if remove_unselected:
		for dt in _doctype_and_children(doctype):
			if dt not in fieldnames_by_doctype and (folder_path / f"{frappe.scrub(dt)}.json").exists():
				doctypes_to_process.append(dt)

	files: list[dict[str, Any]] = []

	# Parent first, then child tables, for stable output ordering.
	ordered_doctypes = [doctype, *sorted(dt for dt in doctypes_to_process if dt != doctype)]
	...
	for dt in ordered_doctypes:
		...
		is_empty = not (merged["custom_fields"] or merged["property_setters"] or merged["custom_perms"])
...
def get_current_customizations(module: str, doctype: str) -> dict[str, Any]:
	...
	for dt in _doctype_and_children(doctype):
		...
		custom_fields = [row for row in (data.get("custom_fields") or []) if isinstance(row, dict)]
		property_setters = [row for row in (data.get("property_setters") or []) if isinstance(row, dict)]

		cf_by_fieldname = {row.get("fieldname"): row for row in custom_fields if row.get("fieldname")}
		...
		files.append(
			{
				...
				"link_count": len([r for r in (data.get("links") or []) if isinstance(r, dict)]),
...
def update_current_customizations(
	...
	for dt in _doctype_and_children(doctype):
		...
		original_cf = [row for row in (data.get("custom_fields") or []) if isinstance(row, dict)]
		original_ps = [row for row in (data.get("property_setters") or []) if isinstance(row, dict)]

		kept_cf = [row for row in original_cf if _encode_target(dt, row.get("fieldname") or "") in keep]
	...
	return {
		...
		"doctype": doctype,
		"results": results,
		"total_removed_custom_fields": sum(r["removed_custom_fields"] for r in results),
	}
...
def get_field_app_matrix(doctype: str) -> dict[str, Any]:
	...
	fields.sort(key=lambda f: (not f["is_duplicate"], f["doctype"], f["fieldname"]))
...
def _prune_field_from_file(file_path: Path, doctype: str, fieldname: str) -> dict[str, Any] | None:
	...
	kept_ps = [
		...
		if not (r.get("doctype_or_field") == "DocField" and r.get("field_name") == fieldname)
	]
...
@frappe.whitelist()
def reconcile_field_apps(doctype: str, keep_map: dict[str, str] | str) -> dict[str, Any]:
...
def _load_existing_customization_file(
	*,
	...
	if not isinstance(existing_data, dict):
		frappe.throw(_("Existing customization file must contain a JSON object: {0}").format(file_path))

	existing_doctype = existing_data.get("doctype")

	if existing_doctype and existing_doctype != doctype:
		frappe.throw(
			_("Existing file belongs to DocType {0}, not {1}.").format(
	...
	existing_data["sync_on_migrate"] = cint(sync_on_migrate)
...
def _merge_customization_data(
	*,
	...
	merged_data["doctype"] = incoming_data["doctype"]

	merged_data["sync_on_migrate"] = incoming_data["sync_on_migrate"]

	merged_data["custom_fields"] = _upsert_records(
	...
	merged_data["property_setters"] = _upsert_records(
		existing_records=existing_data.get(
			"property_setters",
			[],
		),
	...
	if incoming_data.get("custom_perms"):
		merged_data["custom_perms"] = _upsert_records(
			existing_records=existing_data.get(
				"custom_perms",
				[],
			),
			incoming_records=incoming_data["custom_perms"],
			key_function=_custom_permission_key,
		)
	else:
		merged_data["custom_perms"] = existing_data.get(
			"custom_perms",
			[],
		)
...
def _upsert_records(
	*,
	...
	"""
	result = [dict(record) for record in (existing_records or []) if isinstance(record, dict)]
	...
	for record in incoming_records or []:
		...
		record_key = key_function(normalized_record)

		if not record_key:
			continue

		if record_key in record_index:
			existing_index = record_index[record_key]

			result[existing_index] = normalized_record
		else:
			record_index[record_key] = len(result)
...
def _parse_selected_fields(
	...
	if isinstance(custom_fields, str):
		try:
			custom_fields = frappe.parse_json(custom_fields)

		except Exception:
			custom_fields = [value.strip() for value in custom_fields.split(",") if value.strip()]

	if not isinstance(
		custom_fields,
		(list, tuple),
	):
		frappe.throw(_("Custom Fields must be supplied as a list."))
...
def _validate_module(
	...
	if not app_name:
		frappe.throw(_("Module {0} does not exist or has no owning app.").format(frappe.bold(module)))

	if app_name not in frappe.get_installed_apps():
		frappe.throw(_("The app owning module {0} is not installed.").format(frappe.bold(module)))
...
def _validate_access(
	...
	if require_developer_mode and not frappe.conf.developer_mode:
		frappe.throw(_("Customizations can only be exported in developer mode."))
