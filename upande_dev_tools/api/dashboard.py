...
from frappe.utils import format_datetime, get_datetime, today
...
def get_snapshots_today():
	...
	return frappe.db.count("Code Backup Snapshot", filters={"snapshot_time": [">=", today()]})
...
def get_last_backup_display():
	...
	last = frappe.get_all(
		"Code Backup Snapshot", fields=["snapshot_time"], order_by="snapshot_time desc", limit=1
	)
...
def get_activity_rows():
	...
	rows = frappe.get_all(
		...
		fields=["activity_time", "activity_type", "title", "status", "source"],
...
def get_error_rows():
	...
	rows = frappe.get_all(
		...
		fields=["activity_time", "title", "description", "status"],
		order_by="activity_time desc",
		limit_page_length=5,
	)
...
def get_difference_count(issue_type):
	...
	return frappe.db.count("Field Difference Log", filters={"issue_type": issue_type})
...
def get_field_differences():
	...
	return frappe.get_all(
		...
		fields=["name", "doctype_name", "field_name", "issue_type", "live_value", "local_value", "status"],
