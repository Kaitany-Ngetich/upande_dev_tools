...
def save_snapshot(source_type, document_name, content, app=None, module=None):
	...
	last = frappe.get_all(
		...
		filters={"source_type": source_type, "document_name": document_name},
...
def run_code_backup():
	...
	if not is_backup_due(settings):
		log_activity("Backup", "Code backup not due", "Backup interval has not elapsed", "Success")
	...
	if saved == 0:
		log_activity(
			"Backup", "Code backup checked", "No changes detected, no new snapshots saved", "Success"
		)
	else:
...
def get_settings():
	defaults = {"enabled": 1, "include_doctypes": 1, "include_server_scripts": 1, "include_client_scripts": 1}
	...
	try:
		...
		return {
			...
			"include_server_scripts": doc.include_server_scripts
			if doc.include_server_scripts is not None
			else 1,
			"include_client_scripts": doc.include_client_scripts
			if doc.include_client_scripts is not None
			else 1,
...
def get_app_from_module(module_name):
	...
	return frappe.db.get_value("Module Def", {"name": module_name}, "app_name")


def backup_doctypes():
	count = 0

	rows = frappe.get_all("DocType", fields=["name", "module", "app"], limit_page_length=1000)
...
def backup_server_scripts():
	...
	rows = frappe.get_all("Server Script", fields=["name"], limit_page_length=1000)
	...
	for row in rows:
		try:
			...
			if save_snapshot("Server Script", doc.name, content, app, doc.reference_doctype):
				count += 1
...
def backup_client_scripts():
	...
	rows = frappe.get_all("Client Script", fields=["name", "dt", "script", "enabled"], limit_page_length=1000)
