			...
			frm.add_custom_button(__("Run Version Check"), function () {
				...
				frappe.call({
					...
					callback: function (r) {
						if (r.exc) {
							frappe.msgprint(
								__("Version check failed. Please check the error logs.")
							);
							return;
						}
						...
						if (r.message) {
							...
							frm.set_value(
								...
								r.message.has_uncommitted_changes
							);
			...
			frm.add_custom_button(__("Fetch Remote Updates"), function () {
				frappe.call({
					...
					callback: function (r) {
						if (r.exc) {
							frappe.msgprint(
								__("Remote fetch check failed. Please check the error logs.")
							);
							return;
						}
						...
						if (r.message) {
							...
							frappe.show_alert({
								message: __("Remote fetch check completed: {0}", [
									r.message.status,
								]),
