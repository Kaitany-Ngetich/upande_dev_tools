...
def get_settings():
	if not frappe.db.exists("DocType", "Code Backup Settings"):
		return {"enabled": 1, "include_doctypes": 1, "include_server_scripts": 1, "include_client_scripts": 1}

	doc = frappe.get_single("Code Backup Settings")

	return {
		"enabled": doc.enabled,
		"include_doctypes": doc.include_doctypes,
...
def backup_server_scripts():
	...
	rows = frappe.get_all(
		"Server Script", fields=["name", "script_type", "reference_doctype", "script"], limit_page_length=1000
	)
