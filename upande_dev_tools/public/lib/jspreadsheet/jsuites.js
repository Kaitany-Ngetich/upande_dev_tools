...
(function (global, factory) {
	...
	jSuites.calendar = function (el, options) {
		...
		obj.getDays = function () {
			...
			var isCurrentMonthAndYear =
				date.getMonth() == month - 1 && date.getFullYear() == year ? true : false;
	...
	jSuites.calendar.extractDateFromString = function (date, format) {
		...
		if (
			...
			o.values[o.values.length - 1].length >= o.tokens[o.tokens.length - 1].length
		) {
			complete = true;
		}
			...
			if (!container.classList.contains("jcolor-focus")) {
				...
				if (availableWidth < 800 || obj.options.fullscreen == true) {
					...
					if (rectContent.left + rectContent.width > availableWidth) {
						content.style.marginLeft =
							-1 * (rectContent.left + rectContent.width - (availableWidth - 20)) +
							"px";
					}
					if (rectContent.top + rectContent.height > availableHeight) {
						content.style.marginTop =
							-1 * (rectContent.top + rectContent.height - (availableHeight - 20)) +
							"px";
					}
				}
	...
	jSuites.dropdown = function (el, options) {
		...
		obj.find = function (str) {
			...
			if (obj.options.remoteSearch == true) {
				...
				// URL
				var url =
					obj.options.url + (obj.options.url.indexOf("?") > 0 ? "&" : "?") + "q=" + str;
				// Remote search
				jSuites.ajax({
					url: url,
					method: "GET",
		...
		var parser = {
			...
			"0{1}(.{1}0+)?": function (v) {
				...
				if (parseInt(v) >= 0 && parseInt(v) < 10) {
					...
					if (
						(this.values[this.index] != "0" && this.values[this.index] != "-0") ||
						v == decimal
					) {
						this.values[this.index] += v;
					}
