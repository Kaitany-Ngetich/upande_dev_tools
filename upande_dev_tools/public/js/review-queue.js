...
upande_dev_tools.ReviewQueue = class ReviewQueue {
			...
			<div class="dpx-board">
				<div class="dpx-bb-tb">
					...
					<div class="dpx-bb-tb-cmd">
						...
						<span class="dpx-bb-hint">Set a project and priority, pick who does it, then accept</span>
					</div>
				</div>
	...
	visible() {
		...
		return this.requests.filter((r) =>
			[r.title, r.name, r.product_area, r.raised_by_user]
				.filter(Boolean)
	...
	render() {
		...
		$(this.wrapper)
			.find(".dpx-bb-status")
			.html(
				`<b>${rows.length}</b> awaiting review<span>·</span><b>${this.people.length}</b> ${
			...
			return this.stage().html(
				blank(
					...
					this.q
						...
						: "Requests raised from the portal land here for a decision."
				)
			);
		}
		...
		this.stage().html(
			`<div class="dpx-card"><div class="dpx-card-body dpx-bb-listwrap" style="padding:0 0 4px">
				<table class="dpx-bb-table rq-table">
					<colgroup><col><col style="width:96px"><col style="width:150px"><col style="width:170px"><col style="width:120px"><col style="width:150px"><col style="width:210px"></colgroup>
					<thead><tr>
						<th>Request</th><th>Type</th><th>Raised by</th>
	...
	row(r) {
		...
		return `
			<tr class="dpx-bb-row" data-name="${rq_esc(r.name)}">
				<td><div class="subj"><span class="dpx-bb-src">REQ</span>
					<a href="/app/request/${encodeURIComponent(r.name)}">${rq_esc(r.title)}</a></div></td>
				<td>${rq_esc(r.request_type || "—")}</td>
				<td>${rq_esc(r.raised_by_user || "—")}</td>
				...
				<td class="rq-decide">
					<button class="dpx-bb-btn primary rq-act" data-act="accept">Accept</button>
					<button class="dpx-bb-btn rq-act" data-act="Defer">Defer</button>
					<button class="dpx-bb-btn danger rq-act" data-act="Reject">Reject</button>
				</td>
			</tr>`;
	}
