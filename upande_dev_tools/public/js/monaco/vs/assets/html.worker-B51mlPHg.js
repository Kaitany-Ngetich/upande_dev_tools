...
`)
		...
		constructor(e) {
			(this._size = 0),
				...
				(this._perfMon = this._options?._profName
					? new Cn(this._options._profName)
					: void 0),
			...
			const s = new Ee({
				...
				onDidRemoveLastListener: () => {
					this._pendingEmitters.delete(r),
						this._send(new jo(this._workerId, r)),
						(r = null);
				},
			});
			return s.event;
			...
			do {
				...
				S === P || (S < b && u[S - 1] < u[S + 1])
					...
					: ((d = u[S - 1] + 1),
					  ...
					  (g = S - 1 - e)),
					N >= 0 &&
						((u = this.m_forwardHistory[N]), (e = u[0]), (P = 1), (b = u.length - 1));
			} while (--N >= -1);
		...
		ComputeRecursionPoint(e, n, i, r, s, a, l) {
			...
			for (let g = 1; g <= w / 2 + 1; g++) {
				...
				if (
					...
					!this.ContinueProcessingPredicate(v, S)
				)
					return (
						(l[0] = !0),
						(s[0] = v),
	...
	let K = class tt {
		...
		j = class re {
			...
			static lift(e) {
				return e
					? new re(e.startLineNumber, e.startColumn, e.endLineNumber, e.endColumn)
					: null;
			}
			static isIRange(e) {
				return (
		...
		static _createLink(e, n, i, r, s) {
			...
			if (r > 0) {
				const l = n.charCodeAt(r - 1),
					o = n.charCodeAt(a);
				((l === 40 && o === 41) || (l === 91 && o === 93) || (l === 123 && o === 125)) &&
					a--;
			}
			return {
				range: {
	...
	const yn = class yn {
		...
		valueSetReplace(e, n, i) {
			let r = e.indexOf(n);
			return r >= 0
				? ((r += i ? 1 : -1), r < 0 ? (r = e.length - 1) : (r %= e.length), e[r])
				: null;
		}
	};
	yn.INSTANCE = new yn();
	...
	class nl {
		...
		cancel() {
			this._token
				? this._token instanceof Qt && this._token.cancel()
				: (this._token = Jt.Cancelled);
		}
		dispose(e = !1) {
			e && this.cancel(),
				this._parentListener?.dispose(),
				this._token
					? this._token instanceof Qt && this._token.dispose()
					: (this._token = Jt.None);
		}
	}
	class qn {
	...
	const ue = {
			...
			normalize(t) {
				...
				if (
					...
					if (a.length >= 2 && He(a.charCodeAt(0)) && a.charCodeAt(1) === ze)
						return `.\\${a}`;
					let l = t.indexOf(":");
			...
			relative(t, e) {
				...
				return (
					...
					f.length > 0
						? `${f}${i.slice(l, o)}`
						: (i.charCodeAt(l) === de && ++l, i.slice(l, o))
				);
			},
			toNamespacedPath(t) {
	...
	class ge extends j {
		...
		static selectionsArrEqual(e, n) {
			...
			for (let i = 0, r = e.length; i < r; i++)
				if (!this.selectionsEqual(e[i], n[i])) return !1;
			return !0;
		}
	...
	(function (t) {
		...
		function n(s) {
			...
			return (
				a ||
					(console.info("No codicon found for SymbolKind " + s), (a = B.symbolProperty)),
				a
			);
		}
		t.toIcon = n;
		...
		function r(s) {
			...
			return (
				a === void 0 &&
					(console.info("No completion kind found for SymbolKind " + s), (a = 20)),
				a
			);
		}
	...
	class Dl {
		...
		forEach(e, n) {
			...
			for (; r; ) {
				if (
					(n ? e.bind(n)(r.value, r.key, this) : e(r.value, r.key, this),
					this._state !== i)
				)
				...
				case 3: {
					const s = e.codePointAt(0),
						...
						l = St.getLocales().filter(
							(o) =>
								!St.getInstance(new Set([...n.allowedLocales, o])).isAmbiguous(s)
						);
					return {
						kind: 0,
	...
	const Tt = class Tt {
		...
		findLastMaxBy(e) {
			let n,
				i = !0;
			return (
				this.iterate((r) => ((i || ni.isGreaterThan(e(r, n))) && ((i = !1), (n = r)), !0)),
				n
			);
		}
	};
	...
	const _e = class _e {
		...
		static join(e) {
			...
			for (let r = 1; r < e.length; r++)
				(n = Math.min(n, e[r].startLineNumber)),
		...
		constructor(e, n) {
			if (e > n)
				throw new le(`startLineNumber ${e} cannot be after endLineNumberExclusive ${n}`);
	...
	class De {
		...
		addRange(e) {
			...
			const n = ii(
					...
					(r) => r.endLineNumberExclusive >= e.startLineNumber
				),
				i =
					ct(
						this._normalizedRanges,
		...
		subtractFrom(e) {
			...
			return (
				s < e.endLineNumberExclusive && r.push(new X(s, e.endLineNumberExclusive)),
				new De(r)
			);
		}
		toString() {
...
` &&
	...
	class tu {
		...
		getTransformer() {
			return (
				this._transformer || (this._transformer = new eu(this.getValue())),
				this._transformer
			);
		}
		getLineAt(e) {
...
`
				...
				r = zo(n, i),
				s = Rt.ofText(n.substring(0, n.length - r)).addToPosition(
	...
	class ve {
		static inverse(e, n, i) {
			...
			for (const o of e) {
				const u = new ve(
					new X(s, o.original.startLineNumber),
					new X(a, o.modified.startLineNumber)
				);
	...
	function vs(t, e, n, i = !1) {
		...
		for (const s of Gl(
			t.map((a) => iu(a, e, n)),
			(a, l) =>
				a.original.intersectsOrTouches(l.original) ||
				a.modified.intersectsOrTouches(l.modified)
		)) {
			const a = s[0],
				l = s[s.length - 1];
			r.push(
	...
	function iu(t, e, n) {
		...
		t.modifiedRange.endColumn === 1 &&
			...
			t.modifiedRange.startColumn - 1 >= n.getLineLength(t.modifiedRange.startLineNumber) &&
				...
				(i = 1);
		const s = new X(
				t.originalRange.startLineNumber + i,
		...
		static createFromDiffResult(e, n, i, r, s, a, l) {
			...
			if (
				...
				if (f.getElements().length > 0 && w.getElements().length > 0) {
					...
					l && (p = ou(p)), (m = []);
					for (let x = 0, T = p.length; x < T; x++)
						m.push(mt.createFromDiffChange(p[x], f, w));
				}
			}
					...
					if (m !== f) {
						{
							...
							(w > 1 || p > 1) &&
								this._pushTrimWhitespaceCharChange(r, s + 1, 1, w, a + 1, 1, p);
						}
						{
							let w = si(m, 1),
								p = si(f, 1);
							...
							(w < x || p < T) &&
								this._pushTrimWhitespaceCharChange(r, s + 1, w, x, a + 1, p, T);
						}
					}
					s++, a++;
				}
	...
	class ne {
		...
		deltaStart(e) {
			return e === 0
				? this
				: new ne(this.seq1Range.deltaStart(e), this.seq2Range.deltaStart(e));
		}
		deltaEnd(e) {
...
`);
			...
			for (let w = 0; w < e.length; w++)
				for (let p = 0; p < n.length; p++) {
					...
					if (A === y) {
						...
						A === x
							? (l.set(w, p, 0), a.set(w, p, 1))
							: A === T && (l.set(w, p, 0), a.set(w, p, 2));
					s.set(w, p, A);
				}
			const o = [];
			let u = e.length,
	...
	class an {
		...
		findSubWordContaining(e) {
			...
			let i = e;
			for (; i < this.elements.length && ft(this.elements[i]) && !As(this.elements[i]); )
				i++;
			return new G(n, i);
		}
		countLinesIn(e) {
		...
		extendToFullLines(e) {
			const n = ut(this.firstElementOffsetByLineIdx, (r) => r <= e.start) ?? 0,
				i =
					Zl(this.firstElementOffsetByLineIdx, (r) => e.endExclusive <= r) ??
...
`).length >= 15 && pu(d, (f) => f.length >= 2) >= 2
		...
		for (const m of t) {
			...
			for (
				...
				l.forEach(p, ({ range: y }) => {
					for (const I of f)
						if (
							I.originalLineRange.endLineNumberExclusive + 1 ===
								y.endLineNumberExclusive &&
							I.modifiedLineRange.endLineNumberExclusive + 1 ===
		...
		for (let m = 0; m < a.length; m++) {
			const f = a[m],
				w = d.findLastMonotonous(
					(P) => P.original.startLineNumber <= f.original.startLineNumber
				),
			...
			H > 0 &&
				...
				(I > 0 || H > 0) &&
					(a[m] = new ve(
						new X(
							f.original.startLineNumber - I,
							f.original.endLineNumberExclusive + H
						),
						new X(
							f.modified.startLineNumber - I,
							f.modified.endLineNumberExclusive + H
						)
					));
		}
		return a;
		...
		for (let s = 1; s < n.length; s++) {
			...
			if (l.seq1Range.isEmpty || l.seq2Range.isEmpty) {
				...
				for (
					...
					u <= o &&
					!(
						t.getElement(l.seq1Range.start - u) !==
							t.getElement(l.seq1Range.endExclusive - u) ||
						e.getElement(l.seq2Range.start - u) !==
							e.getElement(l.seq2Range.endExclusive - u)
					);
					u++
				);
				if ((u--, u === o)) {
	...
	function Ns(t, e, n, i, r = !1) {
		...
		function o(c, d) {
			...
			for (; s.length > 0; ) {
				const y = s[0];
				if (!(y.seq1Range.intersects(w.seq1Range) || y.seq2Range.intersects(w.seq2Range)))
					break;
		...
		do {
			...
			for (let o = 1; o < i.length; o++) {
				let d = function (f, w) {
					...
					return (
						Math.pow(
							Math.pow(S(y * 40 + A), 1.5) + Math.pow(S(I * 40 + H), 1.5),
							1.5
						) +
							Math.pow(
								Math.pow(S(P * 40 + b), 1.5) + Math.pow(S(g * 40 + v), 1.5),
								1.5
							) >
						(N ** 1.5) ** 1.5 * 1.3
					);
...
`);
					...
					if (s)
						for (let g = 0; g < b; g++) {
							const v = T + g,
								N = y + g;
							if (e[v] !== n[N]) {
								const S = this.refineDiff(
									e,
									n,
									new ne(new G(v, v + 1), new G(N, N + 1)),
									r,
									s,
									i
								);
	...
	let on =
		((Q = class {
			...
			lighten(e) {
				return new Q(
					new Te(this.hsla.h, this.hsla.s, this.hsla.l + this.hsla.l * e, this.hsla.a)
				);
			}
			darken(e) {
				return new Q(
			...
			opposite() {
				return new Q(
					new C(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a)
				);
			}
			mix(e, n = 0.5) {
			...
			toString() {
				return (
					this._toString || (this._toString = Q.Format.CSS.format(this)), this._toString
				);
			}
			toNumber32Bit() {
				return (
	...
	(function (t) {
		(function (e) {
			(function (n) {
				...
				function s(p) {
					return p.hsla.a === 1
						? `hsl(${p.hsla.h}, ${Math.round(p.hsla.s * 100)}%, ${Math.round(
								p.hsla.l * 100
						  )}%)`
				...
				n.formatHSL = s;
				function a(p) {
					return `hsla(${p.hsla.h}, ${Math.round(p.hsla.s * 100)}%, ${Math.round(
						p.hsla.l * 100
					)}%, ${p.hsla.a.toFixed(2)})`;
				}
				n.formatHSLA = a;
				...
				function u(p, x = !1) {
					return x && p.rgba.a === 1
						...
						: `#${l(p.rgba.r)}${l(p.rgba.g)}${l(p.rgba.b)}${l(
								Math.round(p.rgba.a * 255)
						  )}`;
				}
				n.formatHexA = u;
				function c(p) {
				...
				function f(p) {
					...
					if (x === 5) {
						const T = w(p.charCodeAt(1)),
							...
							I = w(p.charCodeAt(4));
						return new t(
							new C(16 * T + T, 16 * y + y, 16 * A + A, (16 * I + I) / 255)
						);
					}
					return null;
				}
	...
	function Eu(t) {
		return !t || typeof t.getValue != "function" || typeof t.positionAt != "function"
			? []
			: Cu(t);
	}
	const Ru = /^-+|-+$/g,
...
`) + 1,
				...
				(g.text || g.hasSeparatorLine) &&
					(n.length === 0 ||
						n[n.length - 1].range.endLineNumber < g.range.startLineNumber) &&
					n.push(g),
		...
		constructor(e, n) {
			(this._onReturn = n),
				...
				(this._iterator = {
					...
					return: () => (
						this._onReturn?.(), Promise.resolve({ done: !0, value: void 0 })
					),
	...
	class Fu {
			...
			for (const i of n)
				this._acceptDeleteRange(i.range),
					this._acceptInsertText(
						new K(i.range.startLineNumber, i.range.startColumn),
						i.text
					);
		...
		_setLineText(e, n) {
			(this._lines[e] = n),
				this._lineStarts &&
					this._lineStarts.setValue(e, this._lines[e].length + this._eol.length);
		}
			...
			e: for (const o of e) {
				...
				if (u) {
					for (const c of u.words(a))
						if (
							!(c === n || !isNaN(Number(c))) &&
							(l.add(c), l.size > et._suggestionsLimit)
						)
							break e;
				}
			}
	...
	(function (t) {
		function e(i, r, s) {
			const a = { label: i };
			return (
				r !== void 0 && (a.needsConfirmation = r), s !== void 0 && (a.description = s), a
			);
		}
		t.create = e;
		function n(i) {
	...
	(function (t) {
		...
		function n(i) {
			const r = i;
			return (
				E.string(r) || (E.objectLiteral(r) && E.string(r.language) && E.string(r.value))
			);
		}
		t.is = n;
	})(dn || (dn = {}));
		...
		function n(i) {
			...
			return (
				...
				(r.triggerKind === void 0 ||
					r.triggerKind === fn.Invoked ||
					r.triggerKind === fn.Automatic)
			);
		}
		t.is = n;
	})(pa || (pa = {}));
	...
	(function (t) {
		...
		function n(i) {
			const r = i;
			return (
				r != null && J.is(r.range) && (E.string(r.expression) || r.expression === void 0)
			);
		}
		t.is = n;
	})(Sa || (Sa = {}));
...
` &&
		...
		update(e, n) {
			for (let i of e)
				if (Ht.isIncremental(i)) {
					...
					if (o - l === c.length)
						for (let m = 0, f = c.length; m < f; m++) u[m + l + 1] = c[m];
					else
						c.length < 1e4
							? u.splice(l + 1, o - l, ...c)
							: (this._lineOffsets = u =
									u.slice(0, l + 1).concat(c, u.slice(o + 1)));
					const d = i.text.length - (a - s);
					if (d !== 0)
						for (let m = l + 1 + c.length, f = u.length; m < f; m++) u[m] = u[m] + d;
	...
	function Ha(t, e, n = 0) {
		...
		for (let r = 0; r < t.length; r++) {
			...
			(s === 13 || s === 10) &&
				(s === 13 && r + 1 < t.length && t.charCodeAt(r + 1) === 10 && r++,
				i.push(n + r + 1));
		}
		return i;
	}
	...
	(function (t) {
		t.LATEST = {
			textDocument: {
				completion: {
					completionItem: { documentationFormat: [Fe.Markdown, Fe.PlainText] },
				},
	...
	function be(t, e = 0, n = V.WithinContent, i = !1) {
		...
		function x() {
			...
			return I !== z.EOS &&
				y === r.pos() &&
				!(i && (I === z.StartTagClose || I === z.EndTagClose))
				? (console.warn(
			...
			switch (s) {
				...
				case V.WithinEndTag:
					...
					if (i && r.peekChar() === gn)
						return (
							(s = V.WithinContent),
							p(y, z.EndTagClose, Se("Closing bracket missing."))
						);
					...
					return (
						...
						c.length > 0
							...
							? p(
									y,
									z.Whitespace,
									Se("Tag name must directly follow the open bracket.")
							  )
							: ((s = V.WithinTag),
							  r.advanceUntilChar(Me),
				...
				case V.WithinTag:
					return r.skipWhitespace()
						...
						: i && r.peekChar() === gn
						? ((s = V.WithinContent),
						  p(y, z.StartTagClose, Se("Closing bracket missing.")))
		...
		async providePathSuggestions(e, n, i, r) {
			...
			if (a)
				try {
					...
					for (const [u, c] of o)
						u.charCodeAt(0) !== gc && l.push(vc(u, c === Ci.Directory, n));
					return l;
				} catch {}
			return [];
		}
...
\r`.indexOf(M) >= 0
				...
				for (R && (U = U.parent); U; ) {
					...
					if (O && (!U.closed || (U.endTagStart && U.endTagStart > d))) {
						...
						if (Z !== null && xe !== null && Z !== xe) {
							const Ce = Z + "</" + O + F;
							(Y.textEdit = oe.replace(x(k - 1 - xe.length), Ce)),
								(Y.filterText = xe + "</" + O);
						}
			...
			function N(k) {
				return d === f.getTokenEnd() &&
					((D = f.scan()), D === k && f.getTokenOffset() === d)
					? f.getTokenEnd()
					: d;
			}
			function S() {
				for (const k of a)
					k.onHtmlContent && k.onHtmlContent({ document: e, position: n });
				return _();
			}
			function _() {
				let k = d - 1,
				...
				for (; d !== z.EOS && c.getTokenEnd() <= s; ) {
					if (d === z.AttributeName && c.getTokenEnd() === s - 1)
						return (
							(d = c.scan()),
							d !== z.DelimiterAssign ||
							((d = c.scan()), d === z.Unknown || d === z.AttributeValue)
								? null
								: o
						);
					d = c.scan();
	...
	class Ac {
		...
		doHover(e, n, i, r) {
			...
			const c = this.dataManager
				.getDataProviders()
				.filter((b) => b.isApplicable(e.languageId));
			function d(b, g, v) {
				for (const N of c) {
					let S = null;
					if (
			...
			function p(b, g) {
				...
				for (
					;
					N !== z.EOS && (v.getTokenEnd() < l || (v.getTokenEnd() === l && N !== b));
...
`);
					...
					function a(o, u) {
						(this.__cache = [""]),
							...
							(u = u || ""),
							o.indent_level > 0 &&
								(u = new Array(o.indent_level + 1).join(this.__indent_string)),
				...
				function (r) {
					...
					(a.prototype.restart = function () {
						...
						(a.prototype.lookBack = function (l) {
							...
							return (
								o >= l.length &&
								this.__input.substring(o - l.length, o).toLowerCase() === l
							);
						}),
...
`
						...
						(y.prototype.eatWhitespace = function (A) {
							for (
								var I = w.test(this._input.peek()), H = 0;
							...
							for (
								...
								if (this._ch === "/" && this._input.peek() === "*") {
									...
									if ((this.preserveSingleSpace(k), this._input.peek() === "{"))
										...
										U.match(/[ :]$/) &&
											...
											P === 0 && U.indexOf(":") !== -1
												...
												? ((this._nestedLevel += 1),
												  U in this.CONDITIONAL_GROUP_RULE && (v = !0))
												: P === 0 && !g && (N = !0);
									}
								else if (this._ch === "#" && this._input.peek() === "{")
									...
									g && ((g = !1), this.outdent()),
										...
										v
											? ((v = !1),
											  (b = this._indentLevel >= this._nestedLevel))
									...
									for (
										var O = 0;
										O < this.NON_SEMICOLON_NEWLINE_PROPERTY.length;
										O++
									)
										if (
											this._input.lookBack(
												this.NON_SEMICOLON_NEWLINE_PROPERTY[O]
											)
										) {
											L = !0;
											break;
...
You passed in: '` +
				...
				function (r, s, a) {
					...
					(m.prototype.tokenize = function () {
						...
						for (
							var f, w = new o(d.START, ""), p = null, x = [], T = new u();
							w.type !== d.EOF;
				...
				function (r) {
					...
					(s.prototype.restart = function () {
						...
						(s.prototype.next = function () {
							...
							return (
								this.hasNext() &&
									((a = this.__tokens[this.__position]), (this.__position += 1)),
								a
							);
						}),
				...
				function (r) {
					...
					(s.prototype.read = function () {
						...
						(s.prototype.starting_with = function (a) {
							var l = this._create();
							return (
								(l._starting_pattern = this._input.get_regexp(a, !0)),
								l._update(),
								l
							);
						}),
						(s.prototype.matching = function (a) {
							var l = this._create();
							return (
								(l._match_pattern = this._input.get_regexp(a, !0)), l._update(), l
							);
						}),
					...
					(f.prototype.current_line_has_match = function (b) {
						...
						(f.prototype.set_space_before_token = function (b, g) {
							(this._output.space_before_token = b),
								(this._output.non_breaking_space = g);
						}),
						...
						(A.prototype._handle_control_flow_open = function (b, g) {
							...
							return (
								b.set_space_before_token(
									g.newlines || g.whitespace_before !== "",
									!0
								),
								g.newlines
...
` +
						...
						(A.prototype._handle_tag_open = function (b, g, v, N, S) {
							...
							if (
								...
								do
									(D = S.peek(L)),
										D.type === c.ATTRIBUTE && (_.attr_count += 1),
										(L += 1);
					...
					var I = function (b, g) {
						if (
							...
							(this.tag_start_char = g.text[0]),
								...
								(this.is_end_tag =
									!this.is_start_tag || (g.closed && g.closed.text === "/>"));
							var N = 2;
							this.tag_start_char === "{" &&
								this.text.length >= 3 &&
								this.text.charAt(2) === "~" &&
					...
					(A.prototype._get_tag_open_token = function (b) {
						...
						return (
							...
							(g.is_end_tag =
								g.is_end_tag || x(g.tag_check, this._options.void_elements)),
							(g.is_empty_element =
								g.tag_complete || (g.is_start_tag && g.is_end_tag)),
							(g.is_unformatted =
								!g.tag_complete && x(g.tag_check, this._options.unformatted)),
							(g.is_content_unformatted =
								!g.is_empty_element &&
								x(g.tag_check, this._options.content_unformatted)),
							(g.is_inline_element =
...
`) === -1) ||
					...
					(A.prototype._do_optional_end_element = function (b) {
						...
						if (!(b.is_empty_element || !b.is_start_tag || !b.parent)) {
							if (b.tag_name === "body") g = g || this._tag_stack.try_pop("head");
								...
								(!v || P.indexOf(v.tag_name) === -1) &&
									(g = g || this._tag_stack.try_pop("p"));
							} else
				...
				function (r, s, a) {
					...
					(w.prototype = new l()),
								...
								A === "!" &&
									...
									T
										? ((y = f.get_directives(T)),
										  y &&
												y.ignore === "start" &&
												(T += f.readIgnored(this._input)))
										: (T = this.__patterns.cdata.read())),
									T &&
										((x = this._create_token(m.COMMENT, T)),
										(x.directives = y));
							}
							return x;
						}),
						...
						(w.prototype._read_control_flows = function (p, x) {
							...
							if (p === "@") {
								...
								p === "}" &&
									...
									((T = this._input.next()),
									(y = this._create_token(m.CONTROL_FLOW_CLOSE, T)));
							return y;
						}),
						...
						(w.prototype._read_attribute = function (p, x, T) {
							...
							if (T && T.text[0] === "<")
								if (p === "=")
									y = this._create_token(m.EQUALS, this._input.next());
								else if (p === '"' || p === "'") {
	...
	function Dc(t, e, n) {
		...
		if (e) {
			...
			(m === i.length || Ga(i, m)) && (d = m),
				(e = J.create(t.positionAt(u), t.positionAt(d)));
...
`.indexOf(t.charAt(e)) !== -1
		...
		var t = {
				470: (r) => {
					...
					var l = {
						resolve: function () {
							for (
								var o, u = "", c = !1, d = arguments.length - 1;
								d >= -1 && !c;
								d--
							) {
								var m;
						...
						relative: function (o, u) {
							...
							for (
								var w = u.length - f, p = m < w ? m : w, x = -1, T = 0;
								T <= p;
								++T
							) {
								if (T === p) {
						...
						basename: function (o, u) {
							...
							if (u !== void 0 && u.length > 0 && u.length <= o.length) {
								...
								return (
									d === m ? (m = p) : m === -1 && (m = o.length), o.slice(d, m)
								);
							}
							for (c = o.length - 1; c >= 0; --c)
						...
						extname: function (o) {
							s(o);
							for (
								var u = -1, c = 0, d = -1, m = !0, f = 0, w = o.length - 1;
								w >= 0;
								--w
							) {
						...
						parse: function (o) {
							...
							m ? ((u.root = "/"), (c = 1)) : (c = 0);
							for (
								var f = -1, w = 0, p = -1, x = !0, T = o.length - 1, y = 0;
								T >= c;
								--T
							)
			...
			function x(S, _, L) {
				...
				for (let R = 0; R < S.length; R++) {
					...
					if (
						...
						W !== void 0
							? (k !== -1 &&
									((D += encodeURIComponent(S.substring(k, R))), (k = -1)),
							  (D += W))
							: k === -1 && (k = R);
					}
				}
				...
				if (
					...
					if (U !== -1) {
						...
						(R = R.substr(U + 1)),
							...
							U === -1
								? (D += L(O, !1, !1))
								: ((D += L(O.substr(0, U), !1, !1)),
								  (D += ":"),
								  (D += L(O.substr(U + 1), !1, !0))),
							(D += "@");
					}
					(R = R.toLowerCase()),
						(U = R.lastIndexOf(":")),
						U === -1
							? (D += L(R, !1, !0))
							: ((D += L(R.substr(0, U), !1, !0)), (D += R.substr(U)));
				}
				if (M) {
					if (M.length >= 3 && M.charCodeAt(0) === 47 && M.charCodeAt(2) === 58) {
						...
						U >= 65 &&
							U <= 90 &&
							(M = `/${String.fromCharCode(U + 32)}:${M.substr(3)}`);
					} else if (M.length >= 2 && M.charCodeAt(1) === 58) {
						const U = M.charCodeAt(0);
						U >= 65 &&
							U <= 90 &&
							(M = `${String.fromCharCode(U + 32)}:${M.substr(2)}`);
					}
					D += L(M, !0, !1);
				}
				return (
			...
			(function (S) {
				(S.joinPath = function (_, ...L) {
					...
					(S.resolvePath = function (_, ...L) {
						...
						let R = g.resolve(D, ...L);
						return (
							k && R[0] === v && !_.authority && (R = R.substring(1)),
							_.with({ path: R })
						);
					}),
	...
	function Fc(t, e) {
		try {
			...
			return (
				n.scheme === "file" &&
					n.query &&
					((n = n.with({ query: null })), (t = n.toString(!0))),
				n.scheme === "file" &&
				n.fragment &&
				...
				switch (s) {
					...
					case z.AttributeValue:
						if (l && a && this.dataManager.isPathAttribute(l, a)) {
							...
							o &&
								typeof u > "u" &&
								((u = Di(d)), u && n && (u = n.resolveReference(u, e.uri))),
								(o = !1),
								(a = void 0);
						} else if (a === "id") {
	...
	function qc(t, e, n) {
		...
		return (
			((a && Ya(a, e)) || (l && Ya(l, e))) &&
				(a && s.push({ kind: mn.Read, range: a }),
				l && s.push({ kind: mn.Read, range: l })),
			s
		);
	}
	function Qa(t, e) {
	...
	function jc(t, e, n, i) {
		...
		const a = [],
			l = {
				start: t.positionAt(s.start + 1),
				end: t.positionAt(s.start + 1 + s.tag.length),
			};
							...
							if (w[1]) l.push({ startLine: m, tagName: "" });
								...
								if (p >= 0) {
									...
									(m = x.startLine),
										T > m &&
											u !== m &&
											c({ startLine: m, endLine: T, kind: cn.Region });
								}
							}
						else {
			...
			return !s.startTagEnd || !s.endTagStart
				...
				: (a.unshift([s.start, s.end]),
				  s.start < r && r < s.startTagEnd
						...
						: (r >= s.endTagStart + 2 && a.unshift([s.endTagStart + 2, s.end - 1]),
						  a));
		}
	...
	class Yc {
		...
		getVoidElements(e) {
			const n = Array.isArray(e)
					? e
					: this.getDataProviders().filter((r) => r.isApplicable(e)),
				i = [];
			return (
				n.forEach((r) => {
					r.provideTags()
	...
	class nh {
		constructor(e, n) {
			...
			const i = this._languageSettings.data,
				...
				s = [];
			if (i?.dataProviders)
				for (const a in i.dataProviders) s.push(th(a, i.dataProviders[a]));
		...
		_getTextDocument(e) {
			...
			for (let i of n)
				if (i.uri.toString() === e)
					return Ai.create(e, this._languageId, i.version, i.getValue());
			return null;
		}
	}
	self.onmessage = () => {
