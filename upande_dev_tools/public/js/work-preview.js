...
upande_dev_tools.attach_preview = function (root) {
	...
	function place(el) {
		...
		if (top + size.height > window.innerHeight - 12) top = Math.max(12, box.top - size.height - GAP);
	...
	function render(d) {
		...
		return `
			${
				d.image
					? `<div class="pv-shot"><img src="${esc(d.image)}" alt="" loading="lazy"></div>`
					: ""
			}
			<div class="pv-body">
				<div class="pv-top">
					...
					${
						d.priority
							? `<span class="dpx-bb-chip pr-${slug(d.priority)}">${esc(d.priority)}</span>`
							: ""
					}
				</div>
