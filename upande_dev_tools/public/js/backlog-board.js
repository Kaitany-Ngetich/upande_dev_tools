...
// The real implementation lives in modal.js (shared with other pages, Dev Dashboard's "New
// deployment request" included) - this just keeps every existing call site in this file
// working unchanged.
function open_modal(title, fields, onSubmit, submitLabel) {
	return upande_dev_tools.open_modal(title, fields, onSubmit, submitLabel);
}
...
upande_dev_tools.BacklogBoard = class BacklogBoard {
	...
	load() {
		...
		if (!this.task_statuses) {
			// Task.status is a plain Select (no master doctype to point at) - read straight
			// from the field's own options instead of a hand-copied list that can drift.
			frappe
				.xcall("upande_dev_tools.api.board.get_task_statuses")
				.then((statuses) => {
					this.task_statuses = statuses && statuses.length ? statuses : TASK_QUICK_STATUSES;
				})
				.catch(() => {
					this.task_statuses = TASK_QUICK_STATUSES;
				});
		}
		frappe
			...
			[
				{
					...
					label: __("Status"),
					type: "select",
					options: (this.task_statuses || TASK_QUICK_STATUSES).map((s) => [s, s]),
					value: item.status,
				},
	...
	mount_sheet(host, rows) {
		...
		this.sheet = jspreadsheet(host, {
			...
			columns: [
				...
				{ type: "dropdown", title: "Status", width: 104, source: this.task_statuses || TASK_QUICK_STATUSES },
