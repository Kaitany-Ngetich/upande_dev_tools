...
- **`Event Participants`' `reference_doctype`/`reference_docname` fields are mandatory** on this Frappe version — a test `Event Participants` row needs both set (e.g. `{"reference_doctype": "User", "reference_docname": <user>, "email": <user>}`), not just `email` alone.
...
```python
	def _make_project(self) -> str:
		...
		# up by the unique `project_name` field rather than by docname — this helper can
		# be called more than once within a single, uncommitted test transaction, and a
		# docname-based exists() check would never match, causing a duplicate-entry error
		# on the second call.
...
```python
	...
	def test_get_my_day_returns_assigned_planned_task_and_events(self) -> None:
		...
		# on this Frappe version (not optional as the schema might suggest) — point
		# them at the dev's own User record.
		event.append(
